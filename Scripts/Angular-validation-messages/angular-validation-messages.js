    ...
    .factory('ValidationMessagesHelper', ['$compile', function ($compile) {
      ...
      return {
        ...
        _getErrorMessage: function (errorKey, inputType, params, opts) {
          ...
          var formatError = function (msg, errorKey, label) {
              if (msg && label && label.length > 0) {
                  if (errorKey === 'required') {
                      msg = msg.indexOf('This field') >= 0 ? msg.replace('This field', label) : msg + ' on ' + label + ' field';
                  } else {
                      msg = msg.replace('This field', label);
                  }
          ...
          if (errorMessageObject) {
              return this.renderError(formatError(errorMessageObject[inputType], errorKey,  opts.label) || formatError(errorMessageObject.default, errorKey,  opts.label) || errorMessageObject, params);
          } else {
